...
//		private final int[][] cFirstNnz; // 每列首次出现的非零元素个数
		...
		public MatrixHistogram(MatrixBlock in, boolean upperBound) {
			...
			if( in.getLength() == in.getNonZeros() ) {
				...
				if( in.isInSparseFormat() ) {
					...
					for( int i=0; i<m; i++ ) {
						if( a.isEmpty(i) ) {
							// if the matrix has an empty row, it will not be diagonal
							fullDiag = false;
							continue;
						}
						int apos = a.pos(i);
				...
				// if the matrix is empty, it will not be diagonal
				fullDiag = false;
			}
		...
		private static MatrixHistogram deriveDiagHistogram(MatrixHistogram h1) {
			if( h1.getCols() == 1 ) { //vector-matrix
				...
				int rMaxNnz = 0; SplittableRandom rand = new SplittableRandom();
				for(int i=0; i<m; i++) {
		...
		// 使用SplittableRandom加速
		private static int probRound(double inNnz, SplittableRandom rand) {
